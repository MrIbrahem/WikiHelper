...
Application Architecture:
    ...
    - Session-based user identification for workspace isolation
    - CSRF protection on all forms
    - Path traversal protection on all file operations

User Isolation:
    Workspaces are isolated per-user based on a session-stored username.
    Each user's workspaces are stored in a subdirectory under WIKI_WORK_ROOT.
    Note: This is NOT authentication - session can be manipulated.

Routes:
    /                   Dashboard listing workspaces
    /new                Create new workspace from WikiText
    /import-wikipedia   Import article from Wikipedia API
    /set_user           Set username in session
    /logout             Clear username from session
...
Security Considerations:
    ...
    - No authentication (session-based user isolation only)
...
def get_user_root() -> Optional[Path]:
    ...
    session can be manipulated by users.

    Returns:
        Path to the user's workspace directory if username session exists
        and is valid.
        None if no username in session or validation fails.
...
def check_user() -> Optional[RouteResponse]:
    ...
    Returns:
        None if username session exists (request proceeds normally).
        Redirect response to set_user if no username in session.

    Exempt Routes:
        - set_user: The route that sets the session
...
def set_user() -> RouteResponse:
    ...
    if request.method == "POST":
        ...
        # Set session as permanent (expires according to PERMANENT_SESSION_LIFETIME)
